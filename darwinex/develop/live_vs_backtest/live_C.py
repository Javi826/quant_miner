#quant_d/develop/live_vs_backtest/live_C.py
import os
import glob
import logging
...
logger = logging.getLogger("live_lab.compare_darwinex")
...
PRODUCTION_CSV   = os.path.join(_BASE, "production_trades.csv")
BATCH_TRADES_DIR = os.path.join(_BASE, "brief_trades")
...
SELECTED_STRATEGIES = [
    "000771_4H_long_RSI7lt60_AND_ADX14gt30",
    "053208_4H_long_RSI14lt40_AND_ADX14gt25_AND_HISTVOL30gtSMA_HISTVOL20",
    "061887_4H_long_RSI14lt50_AND_HISTVOL10gtSMA_HISTVOL40_AND_HISTVOL30gtSMA_HISTVOL20",
]
...
DETAIL_STRATEGY = "000771_4H_long_RSI7lt60_AND_ADX14gt30"

# Batch exit reasons that are artificial (the data range ended before TP/SL was
# hit) and therefore not comparable against a real production close
...
def load_production(path: str) -> pd.DataFrame:
    """Loads production trades. Rows without a close carry status == 'OPEN'."""
    df = pd.read_csv(path)
    ...
    if "status" not in df.columns:
        df["status"] = np.where(df["sell_time"].isna(), "OPEN", "CLOSED")
...
def _short_id(strategy_id: str) -> str:
    """Extracts the leading numeric id from a strategy label (e.g. '000771')."""
...
def main() -> None:
    ...
    df_prod = load_production(PRODUCTION_CSV)
