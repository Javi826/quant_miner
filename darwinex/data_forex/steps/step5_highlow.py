...
def _process_pair(tf_high: str, tf_low: str, file_index: dict, output_dir: str, export_csv: bool) -> int:
...
def run(config: dict) -> bool:
    ...
    for pair in tf_pairs:
        if len(pair) != 2:
            logger.warning(f"⚠ Invalid pair {pair} — must be [higher_tf, lower_tf]. Skipping.")
            continue
