...
def backtest_core(
    ...
    # ── Open-position state (flat arrays, indexed by a free-slot pool of size max_trades — ──
    # ── multiple concurrent positions per symbol are now allowed ──
    cdef np.ndarray[long,   ndim=1] pos_sym_id_arr          = np.empty(max_trades, dtype=np.int64)
    cdef np.ndarray[double, ndim=1] pos_qty_arr              = np.empty(max_trades, dtype=np.float64)
    cdef np.ndarray[double, ndim=1] pos_buy_price_arr        = np.empty(max_trades, dtype=np.float64)
    cdef np.ndarray[long,   ndim=1] pos_buy_time_int_arr     = np.empty(max_trades, dtype=np.int64)
    cdef np.ndarray[long,   ndim=1] pos_sell_time_int_arr    = np.empty(max_trades, dtype=np.int64)
    cdef np.ndarray[double, ndim=1] pos_commission_buy_arr   = np.empty(max_trades, dtype=np.float64)
    cdef np.ndarray[int,    ndim=1] pos_is_short_arr         = np.empty(max_trades, dtype=np.int32)
    cdef np.ndarray[double, ndim=1] pos_blocked_amount_arr   = np.empty(max_trades, dtype=np.float64)
    cdef np.ndarray[int,    ndim=1] pos_has_exec_arr         = np.empty(max_trades, dtype=np.int32)
    cdef np.ndarray[double, ndim=1] pos_exec_price_arr       = np.empty(max_trades, dtype=np.float64)
    cdef np.ndarray[long,   ndim=1] pos_exec_time_int_arr    = np.empty(max_trades, dtype=np.int64)
    cdef np.ndarray[int,    ndim=1] pos_exit_reason_code_arr = np.empty(max_trades, dtype=np.int32)
    ...
    # ── Manual min-heap over (exec_time, counter) -> slot (sized to max_trades: many ──
    # ── positions — possibly several per symbol — can be open at once) ──
    cdef np.ndarray[long, ndim=1] heap_time_arr    = np.empty(max_trades, dtype=np.int64)
    cdef np.ndarray[long, ndim=1] heap_counter_arr = np.empty(max_trades, dtype=np.int64)
    cdef np.ndarray[int,  ndim=1] heap_slot_arr    = np.empty(max_trades, dtype=np.int32)
    ...
    cdef np.ndarray[int, ndim=1] free_slot_arr = np.empty(max_trades, dtype=np.int32)
    cdef int[::1] free_slot = free_slot_arr
    cdef int free_slot_count
    ...
    cdef int    sid, buy_idx, n_bars, exit_idx, slot
    ...
    with nogil:
        ...
        for tick_i in range(n_ticks):
            ...
            while heap_size > 0 and heap_time[0] <= t_int:
                ...
                free_slot[free_slot_count] = slot
                free_slot_count += 1

            # ── 2. Open new positions for every signal at this tick — no longer gated ──
            # ── on all positions being closed; limited only by free_cash & free slots ──
            ...
            while ev_scan < n_events and ev_mv[ev_scan, 0] == t_int:
                ...
                free_slot_count -= 1
                slot = free_slot[free_slot_count]
