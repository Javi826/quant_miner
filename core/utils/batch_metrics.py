...
def _to_trading_days(days: np.ndarray) -> np.ndarray:
    """Snap any non-trading-day timestamp onto the preceding trading day."""
...
def daily_values_from_sell_days(sell_days_ns: np.ndarray, profits: np.ndarray) -> tuple:
    """Bucket per-trade profits into daily P&L totals, on the settings.WEEKMASK calendar.
    ...
    total profit on trading day (start_day + i trading days).
    """
